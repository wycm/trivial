...
<html>
 <head>  
   ...
   <script type="text/javascript" src="admin/js/release.js"></script>
    ...
    <script type="text/javascript" charset="utf-8" src="res/lib/ueditor/ueditor.config.js"></script>
    <script type="text/javascript" charset="utf-8" src="res/lib/ueditor/ueditor.all.min.js"> </script>
    <!--建议手动加在语言，避免在ie下有时因为加载语言失败导致编辑器加载失败-->
    <!--这里加载的语言文件会覆盖你在配置项目里添加的语言类型，比如你在配置项目里配置的是英文，这里加载的中文，那最后就是中文-->
    <script type="text/javascript" charset="utf-8" src="res/lib/ueditor/lang/zh-cn/zh-cn.js"></script>
...
<%@ include file="./pagetop.jsp" %>
	...
	<div id="breadcrumbs" class="con_box clearfix">
				...
				<a href="<c:out value="${sessionScope.user.url}"/>"><c:out value="${sessionScope.user.username}"/></a>
				<a>个人中心</a>
				<a>发布文章</a>
				</div>
	</div> 
    ...
    <div id="user_art_main" class="fl"> 
        ...
        <div class="result-wrap">
            <div class="result-content">
                ...
                <form action="user/add-article" method="post" id="valueform" name="valueform" enctype="multipart/form-data">
                    <table class="insert-tab" width="100%">
                        <tbody><tr>
                            <th width="120"><i class="require-red">*</i>分类：</th>
                            <td>
                                <select name="articletypeId" id="catid" class="required">
                                <c:forEach items="${chiArticleTypes}" var="item">
                    	...
                    	常用标签：
                    		...
                    		<c:forEach items="${tagList}" var="item">
                    			<span id="<c:out value="${item.id}"/>" class="label label-info" name="<c:out value="${item.value}"/>"><c:out value="${item.value}"/></span>
                    		</c:forEach>
               				 </div>
            						</div>
                                </td>
