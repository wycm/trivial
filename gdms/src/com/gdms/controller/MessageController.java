...
public class MessageController extends BaseController{
	...
	@RequestMapping("/unreadAmount")
	public String getunReadAmount(HttpSession session, HttpServletResponse response){
		User user = (User) session.getAttribute("user");
		List<Message> unreadmessageList = messageService.getUnreadMessageListByUser(user);
		int amount = 0;
		if(unreadmessageList!=null){
			amount = unreadmessageList.size();
		}
		String json = "{\"result\":"+amount+"}";
